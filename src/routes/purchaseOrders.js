...
async function nextPONumber() {
  const last = await PurchaseOrder.findOne({ order: [['createdAt', 'DESC']] });
...
router.get('/', authRequired, adminOnly, async (req, res) => {
  try {
    const list = await PurchaseOrder.findAll({ order: [['createdAt', 'DESC']] });
    res.json(list);
...
router.post('/:id/receive', authRequired, adminOnly, async (req, res) => {
  try {
    ...
    for (const ri of receivedItems) {
      const idx = poItems.findIndex(it => String(it._id || it.id) === String(ri.itemId));
      if (idx < 0) continue;
      ...
      let product = poItem.productId ? await Product.findByPk(poItem.productId) : null;
      ...
      if (!product && poItem.isNewProduct) {
        ...
        const variantsWithStock = hasVariants
          ? npd.variants.map(v => ({ _id: crypto.randomUUID(), size: v.size, color: v.color, stock: v.stock || 0 }))
          : [];
        ...
        product = newProd;
        poItem.productId = newProd.id;
      } else if (product) {
        if (poItem.variantId) {
          const variants = [...(product.variants || [])];
          const vIdx = variants.findIndex(v => String(v._id || v.id) === String(poItem.variantId));
          if (vIdx >= 0) {
            variants[vIdx] = { ...variants[vIdx], stock: (variants[vIdx].stock || 0) + qtyReceiving };
      ...
      if (ri.warehouseId) {
        const wh = await Warehouse.findByPk(ri.warehouseId);
        if (wh) {
          warehouseName = wh.name;
          if (ri.shelfId) {
            const shelf = (wh.shelves || []).find(s => String(s._id) === String(ri.shelfId));
    ...
    if (movements.length > 0) await StockMovement.bulkCreate(movements);

    // Update PO status
    const allReceived = poItems.every(it => (it.quantityReceived || 0) >= it.quantityOrdered);
    const someReceived = poItems.some(it => (it.quantityReceived || 0) > 0);
